...
class Process
  ...
  def self.on_interrupt(&handler : ->) : Nil
    System::Process.on_interrupt(&handler)
  end
  ...
  def self.exec(command : String, args = nil, env : Env = nil, clear_env : Bool = false, shell : Bool = false,
    ...
    command_args = System::Process.prepare_args(command, args, shell)

    input = exec_stdio_to_fd(input, for: STDIN)
    output = exec_stdio_to_fd(output, for: STDOUT)
    error = exec_stdio_to_fd(error, for: STDERR)

    System::Process.replace(command_args, env, clear_env, input, output, error, chdir)
  end
  ...
  def initialize(command : String, args = nil, env : Env = nil, clear_env : Bool = false, shell : Bool = false,
    ...
    pid = System::Process.spawn(command_args, env, clear_env, fork_input, fork_output, fork_error, chdir.try &.to_s)
    @process_info = System::Process.new(pid)
  ...
  def signal(signal : Signal) : Nil
    System::Process.signal(@process_info.pid, signal)
  end
