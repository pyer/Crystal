...
class Scheduler
  @event_loop = System::EventLoop.create
  ...
  {% if flag?(:preview_mt) %}
    private getter(fiber_channel : FiberChannel) { FiberChannel.new }
  ...
  private def fatal_resume_error(fiber, message)
    System.print_error "\nFATAL: #{message}: #{fiber}\n"
    caller.each { |line| System.print_error "  from #{line}\n" }
    exit 1
  end
    ...
    private def self.worker_count
      ...
      if env_workers && !env_workers.empty?
        ...
        if !workers || workers < 1
          System.print_error "FATAL: Invalid value for CRYSTAL_WORKERS: #{env_workers}\n"
          exit 1
        end
