...
module System::File
  ...
  def self.utime(atime : ::Time, mtime : ::Time, filename : String) : Nil
    ...
    timevals[0] = System::Time.to_timeval(atime)
    timevals[1] = System::Time.to_timeval(mtime)
  ...
  private def system_utime(atime : ::Time, mtime : ::Time, filename : String) : Nil
    ret = {% if LibC.has_method?("futimens") %}
            ...
            timespecs[0] = System::Time.to_timespec(atime)
            timespecs[1] = System::Time.to_timespec(mtime)
