...
module System::Random
  ...
  private def self.getrandom(buf : UInt8*, buflen : LibC::SizeT, flags : UInt32) : LibC::SSizeT
  ...
  # portable).
  private GRND_NONBLOCK = 1u32

  private def self.sys_getrandom(buf : Bytes)
    loop do
      read_bytes = getrandom(buf.to_unsafe, LibC::SizeT.new(buf.size), GRND_NONBLOCK)
      if read_bytes < 0
