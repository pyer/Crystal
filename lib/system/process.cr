...
struct System::Process
  ...
  def initialize(@pid : LibC::PidT)
    @channel = System::SignalChildHandler.wait(@pid)
  end
  ...
  def exists?
    !@channel.closed? && System::Process.exists?(@pid)
  end

  def terminate(*, graceful)
    System::Process.signal(@pid, graceful ? LibC::SIGTERM : LibC::SIGKILL)
  end
  ...
  def self.start_interrupt_loop : Nil
    # do nothing; `System::Signal.start_loop` takes care of this
  end
  ...
  def self.fork(*, will_exec = false)
    ...
    when 0
      ...
      if will_exec
        ...
        System::Signal.after_fork_before_exec
