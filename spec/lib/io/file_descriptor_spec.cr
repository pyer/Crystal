...
describe IO::FileDescriptor do
  pending "reopen STDIN with the right mode", tags: %w[slow] do
  ...
  pending "opens STDIN in binary mode", tags: %w[slow] do
  ...
  pending "opens STDOUT in binary mode", tags: %w[slow] do
    code = %q(puts "foo")
    compile_source(code) do |binpath|
      io = IO::Memory.new
  ...
  pending "opens STDERR in binary mode", tags: %w[slow] do
